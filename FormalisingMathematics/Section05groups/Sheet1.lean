...
example : a⁻¹ * (a * b) = b := by
  have prop₀ := mul_assoc a⁻¹ a b
  rw [← prop₀]
  have prop₁ := inv_mul_cancel a
  rw [prop₁]
  exact one_mul b
...
example : a * (a⁻¹ * b) = b := by
  have prop₀ := mul_assoc a a⁻¹ b
  rw [← prop₀]
  have prop₁ := mul_inv_cancel a
  rw [prop₁]
  exact one_mul b

theorem un_inv_th {a b c : G} (h1 : b * a = 1) (h2 : a * c = 1) : b = c := by
  -- hint for this one if you're doing it from first principles: `b * (a * c) = (b * a) * c`
  calc
    b = b * (a * c) := by rw [h2] ; apply Eq.symm ; exact MulOneClass.mul_one b
    _ = (b * a) * c := by apply Eq.symm; exact mul_assoc b a c
    _ = _ := by rw [h1] ; exact MulOneClass.one_mul c

theorem inv_crit (a b : G) : a * b = 1 ↔ a⁻¹ = b := by
  constructor
  · intro h
    exact un_inv_th G (inv_mul_cancel a) h
  · intro h
    rw [← h]
    exact mul_inv_cancel a

example : (1 : G)⁻¹ = 1 := by
  have prop₀ := (inv_crit G (1 : G) 1).mp
  apply prop₀
  exact MulOneClass.mul_one 1

example : a⁻¹⁻¹ = a := by
  apply (inv_crit G a⁻¹ a).mp
...
example (h : ∀ g : G, g * g = 1) : ∀ g h : G, g * h = h * g := by
  intro g₀
  intro g₁
  have prop₀ := ((inv_crit G g₀ g₀).mp) (h g₀)
  have prop₁ := ((inv_crit G g₁ g₁).mp) (h g₁)
  calc
    g₀ * g₁ = g₀⁻¹ * g₁⁻¹ := by rw [prop₀, prop₁]
    _ = (g₁ * g₀)⁻¹ := by apply Eq.symm ; exact DivisionMonoid.mul_inv_rev g₁ g₀
    _ = _ := (inv_crit G (g₁ * g₀) (g₁ * g₀)).mp (h (g₁ * g₀))
